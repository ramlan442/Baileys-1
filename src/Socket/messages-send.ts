	...
	return {
		...
		sendMessage: async(
			...
			if(
				...
				if(fullMsg?.message?.messageContextInfo?.messageSecret){
					const option = {}
					for (const value of fullMsg.message.pollCreationMessage!.options!) {
						const hash = sha256(Buffer.from(value.optionName!)).toString('base64');
						option[hash] = value.optionName 
					}
					cache.set(fullMsg.key.id!, {
						messageSecret: fullMsg.message.messageContextInfo.messageSecret,
						option
					}, 600)
				}
